...
export function LongShortAccountsChart({ data, period }: LongShortAccountsChartProps) {
  ...
  const displayData = chartData.length > 0
    ...
    : Array.from({ length: 30 }, (_, i) => {
        ...
        const ratio = 3.0 + (Math.random() - 0.5) * 0.4 // ~3.0
        const shortAccount = 100 / (1 + ratio)
        const longAccount  = 100 - shortAccount

        return {
          time: thaiTime,
          fullTime: `${thaiDate} ${thaiTime}`,
          longShortAccounts: ratio,
          shortAccount,
          longAccount,
        }
      })
  ...
  const processedData = displayData.map((item: any) => {
    const ratio = Number(item.longShortAccounts) || 0
    let { longAccount, shortAccount } = item
    if (longAccount == null || shortAccount == null) {
      if (ratio > 0) {
        shortAccount = 100 / (1 + ratio)
        longAccount = 100 - shortAccount
      } else {
        shortAccount = 0; longAccount = 0
      }
    }
    return {
      ...item,
      longAccount: Number(longAccount),
      shortAccount: Number(shortAccount),
      fullTime: item.fullTime || item.time
    }
  })

  // Domain dynamic + padding 3%
  const values = processedData.map((d: any) => Number(d.longShortAccounts) || 0)
  const minVal = values.length ? Math.min(...values) : 2.5
  const maxVal = values.length ? Math.max(...values) : 3.5
  const padding = (maxVal - minVal) * 0.15
  const domain: [number, number] = [minVal - padding, maxVal + padding]

  const average = processedData.length
    ? processedData.reduce((s: number, it: any) => s + (Number(it.longShortAccounts) || 0), 0) / processedData.length
    : (minVal + maxVal) / 2
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      const ratio = Number(d.longShortAccounts) || 0
      const shortPct = Number(d.shortAccount) || 0
      const longPct  = Number(d.longAccount) || 0
      ...
      return (
        <div className="bg-gray-900 border border-gray-700 rounded-lg p-3 shadow-xl" style={{ backgroundColor:'#1a1a1a', border:'1px solid #374151' }}>
          ...
          <div className="space-y-2 mb-3">
            <div className="flex justify-between"><span className="text-sm text-red-400">Short Account:</span><span className="font-semibold text-red-400">{shortPct.toFixed(2)}%</span></div>
            <div className="flex justify-between"><span className="text-sm text-green-400">Long Account:</span><span className="font-semibold text-green-400">{longPct.toFixed(2)}%</span></div>
          </div>

          {/* ปรับรูปแบบ ratio */}
          <div className="border-t border-gray-700 pt-2">
            <div className="flex justify-between items-center gap-4">
              <span className="text-sm text-gray-300">Long/Short Accounts Ratio:</span>
        ...
        <AreaChart data={processedData} margin={{ top:20, right:60, left:60, bottom:20 }}>
          <defs>
            <linearGradient id="colorAccounts" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%"  stopColor="#0ecef0ff" stopOpacity={0.30}/>
              <stop offset="95%" stopColor="#0ecef0ff" stopOpacity={0.05}/>
            </linearGradient>
          </defs>
          ...
          <YAxis stroke="#0ecef0ff" fontSize={10} tickLine={false} tickCount={3} tickFormatter={formatRatio} domain={domain} />

          <ReferenceLine y={average} stroke="#0ecef0ff" strokeDasharray="3 3" strokeOpacity={0.3} />
          ...
          <Line
            ...
            stroke="#0ecef0ff"
            strokeWidth={3}
            dot={{ r: 2, fill:'#0ecef0ff', stroke:'#fff', strokeWidth:1, cursor:'pointer' }}
            activeDot={{ r:3, fill:'#0ecef0ff', stroke:'#fff', strokeWidth:2 }}
          />
        </AreaChart>
      </ResponsiveContainer>
    </div>
